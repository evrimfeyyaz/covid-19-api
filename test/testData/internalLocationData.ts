import { InternalLocationData } from '../../src/types';

export const internalLocationDataArray: InternalLocationData[] = [
  {
    location: 'US (American Samoa)',
    countryOrRegion: 'US',
    provinceOrState: 'American Samoa',
    latitude: '-14.271000000000000',
    longitude: '-170.132',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: null,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: null,
      },
    ],
  },
  {
    location: 'US (Autauga, Alabama)',
    countryOrRegion: 'US',
    provinceOrState: 'Alabama',
    county: 'Autauga',
    latitude: '32.53952745',
    longitude: '-86.64408227',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: null,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: null,
      },
    ],
  },
  {
    location: 'US (Baldwin, Alabama)',
    countryOrRegion: 'US',
    provinceOrState: 'Alabama',
    county: 'Baldwin',
    latitude: '30.72774991',
    longitude: '-87.72207058',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: null,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: null,
      },
    ],
  },
  {
    location: 'US (Southeast Utah, Utah)',
    countryOrRegion: 'US',
    provinceOrState: 'Utah',
    county: 'Southeast Utah',
    latitude: '38.99617072',
    longitude: '-110.70139580000000',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: null,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: null,
      },
    ],
  },
  {
    location: 'US (Southwest Utah, Utah)',
    countryOrRegion: 'US',
    provinceOrState: 'Utah',
    county: 'Southwest Utah',
    latitude: '37.85447192',
    longitude: '-111.4418764',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: null,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: null,
      },
    ],
  },
  {
    location: 'US (Alabama)',
    countryOrRegion: 'US',
    provinceOrState: 'Alabama',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: null,
      },
      {
        date: '1/23/20',
        confirmed: 8,
        deaths: 2,
        recovered: null,
      },
    ],
    latitude: '32.318230',
    longitude: '-86.902298',
  },
  {
    location: 'US (Utah)',
    countryOrRegion: 'US',
    provinceOrState: 'Utah',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: null,
      },
      {
        date: '1/23/20',
        confirmed: 8,
        deaths: 2,
        recovered: null,
      },
    ],
    latitude: '39.419220',
    longitude: '-111.950684',
  },
  {
    location: 'Australia (Australian Capital Territory)',
    countryOrRegion: 'Australia',
    provinceOrState: 'Australian Capital Territory',
    latitude: '-35.4735',
    longitude: '149.0124',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: 0,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: 2,
      },
    ],
  },
  {
    location: 'Australia (New South Wales)',
    countryOrRegion: 'Australia',
    provinceOrState: 'New South Wales',
    latitude: '-33.8688',
    longitude: '151.2093',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: 0,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: 2,
      },
    ],
  },
  {
    location: 'Canada (Alberta)',
    countryOrRegion: 'Canada',
    provinceOrState: 'Alberta',
    latitude: '53.9333',
    longitude: '-116.5765',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: null,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: null,
      },
    ],
  },
  {
    location: 'Canada (British Columbia)',
    countryOrRegion: 'Canada',
    provinceOrState: 'British Columbia',
    latitude: '49.2827',
    longitude: '-123.1207',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: null,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: null,
      },
    ],
  },
  {
    location: 'China (Anhui)',
    countryOrRegion: 'China',
    provinceOrState: 'Anhui',
    latitude: '31.8257',
    longitude: '117.2264',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: 0,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: 2,
      },
    ],
  },
  {
    location: 'China (Beijing)',
    countryOrRegion: 'China',
    provinceOrState: 'Beijing',
    latitude: '40.1824',
    longitude: '116.4142',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: 0,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: 2,
      },
    ],
  },
  {
    location: 'Turkey',
    countryOrRegion: 'Turkey',
    latitude: '38.9637',
    longitude: '35.2433',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: 0,
      },
      {
        date: '1/23/20',
        confirmed: 4,
        deaths: 1,
        recovered: 2,
      },
    ],
  },
  {
    location: 'Australia',
    countryOrRegion: 'Australia',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: 0,
      },
      {
        date: '1/23/20',
        confirmed: 8,
        deaths: 2,
        recovered: 4,
      },
    ],
    latitude: '-25.274399',
    longitude: '133.775131',
  },
  {
    location: 'Canada',
    countryOrRegion: 'Canada',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: 0,
      },
      {
        date: '1/23/20',
        confirmed: 8,
        deaths: 2,
        recovered: 4,
      },
    ],
    latitude: '56.1304',
    longitude: '-106.3468',
  },
  {
    location: 'China',
    countryOrRegion: 'China',
    values: [
      {
        date: '1/22/20',
        confirmed: 0,
        deaths: 0,
        recovered: 0,
      },
      {
        date: '1/23/20',
        confirmed: 8,
        deaths: 2,
        recovered: 4,
      },
    ],
    latitude: '35.861660',
    longitude: '104.195396',
  },
];
